    ...
    if request.method == "POST":
        ...
        if "scripe_page" in request.POST and scripe_form.is_valid():
            ...
            try:
                ...
                limit = 1
                async_task(
                    "vei_platform.tasks.scripe_factories_list", page_number, limit
                )
                messages.success(
                ...
                messages.error(
                    request, "Form cleaned data = " + str(scripe_form.cleaned_data)
                )

        if "scripe_all" in request.POST:
            page_number = 1
            limit = 10000
            async_task("vei_platform.tasks.scripe_factories_list", page_number, limit)
        ...
        if "scripe_tax_id" in request.POST and scripe_form.is_valid():
            ...
            async_task(
                "vei_platform.tasks.scripe_legal_entity",
                tax_id=tax_id,
