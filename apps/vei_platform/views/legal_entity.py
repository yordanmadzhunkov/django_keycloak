...
def view_entity_detail(request, pk=None):
    ...
    if pk is not None:
        ...
        factories_list = (
            ElectricityFactory.objects.all()
            .filter(primary_owner=legal_entity)
            .order_by("pk")
        )
...
def view_my_entity_detail(request):
    ...
    if instance:
        form = (
            LegalEntityForm(instance=instance)
            if request.method != "POST"
            else LegalEntityForm(request.POST, instance=instance)
        )
    else:
        form = (
            LegalEntityForm()
            if request.method != "POST"
            else LegalEntityForm(request.POST)
        )
        ...
        if form.is_valid():
            ...
            for key in request.POST.keys():
                if key.startswith("add_"):
                    ...
                    if (
                        len(PlatformLegalEntity.objects.filter(entity=entity_to_add))
                        == 0
                    ):
                        ...
                        messages.error(
                            request,
                            _("Can" "t add %s, It" "s already owned by plaform")
                            % str(entity_to_add),
                        )
    platform_entites_list = []
    for e in PlatformLegalEntity.objects.all():
        platform_entites_list.append(
            {
                "native_name": e.entity.native_name,
