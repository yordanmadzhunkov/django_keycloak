...
from vei_platform.models.campaign import (
    Campaign as CampaignModel,
    InvestementInCampaign,
)
...
class Campaign(View):
    ...
    def get(self, request, pk, *args, **kwargs):
        ...
        if request.user and request.user.is_authenticated:
            ...
            if is_manager or is_reviewer:
                return self.get_as_manager_or_reviewer(
                    request, context, is_reviewer=is_reviewer
                )
            else:
    ...
    def get_as_investor(self, request, context):
        ...
        if self.campaign.accept_investments():
            if len(my_investments) == 0:
                context["card_form"] = CreateInvestmentForm(
                    initial={
                        "amount": Money(Decimal("1000"), self.campaign.amount.currency)
                    }
                )
            else:
        ...
        context["factory"] = self.campaign.factory
        context["campaign"] = self.campaign
        context["investors"] = self.campaign.get_investors_for_view(
            show_users=False, investor_profile=profile
        )
        context["hide_link_buttons"] = True
        return render(request, "campaign.html", context)

    def get_as_manager_or_reviewer(self, request, context, is_reviewer):
        context["card_form"] = CampaingEditForm(
            instance=self.campaign, is_reviewer=is_reviewer
        )
        context["factory"] = self.campaign.factory
        context["campaign"] = self.campaign
            ...
            if "extend" in request.POST:
                ...
                if form.is_valid():
                    ...
                    if next_deadline > self.campaign.start_date:
                        ...
                        messages.success(
                            request,
                            _("Campaign deadline extended to %s" % (next_deadline)),
                        )
                    else:
                        messages.error(
                            request,
                            _("Campaign deadline should be further in the future"),
                        )
            ...
            if "complete" in request.POST:
                if self.campaign.allow_finish():
                    ...
                    messages.error(
                        request,
                        _(
                            "Not enough accumulated interest to be able to finish the campaing"
                        ),
                    )
            ...
            if "invest" in request.POST:
                ...
                if form.is_valid():
                    ...
                    if amount.currency == self.campaign.amount.currency:
                        ...
                        messages.success(
                            request,
                            _("You declared interest in amount %s to %s")
                            % (amount, self.campaign.factory.name),
                        )
                    else:
                        messages.error(
                            request,
                            _(
                                "Currency conversion from %s to %s is not supported"
                                % (amount.currency, self.campaign.amount.currency)
                            ),
                        )
                else:
            ...
            if "save" in request.POST:
                ...
                if form.is_valid():
                    ...
                    if amount.currency == self.campaign.amount.currency:
                        ...
                        messages.success(
                            request,
                            _("You declared interest in amount changed to %s")
                            % my_investments[0].amount,
                        )
                    else:
                        messages.error(
                            request,
                            _("Currency conversion from %s to %s is not supported")
                            % (amount.currency, self.campaign.amount.currency),
                        )
    ...
    def post(self, request, pk, *args, **kwargs):
        ...
        if request.user and request.user.is_authenticated:
            is_reviewer = request.user.is_staff
            is_manager = (
                self.campaign.factory.get_manager_profile()
