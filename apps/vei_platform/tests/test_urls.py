...
from vei_platform.views.factory import (
    FactoryCreate,
    FactoryEdit,
    FactoriesList,
    FactoryDetail,
...
class UrlTest(TestCase):
    ...
    def test_my_profile_page_url(self):
        ...
        self.assertEquals(
            resolve(url).func.view_class, MyProfileUpdate.as_view().view_class
        )
    ...
    def test_factory_create_page_url(self):
        ...
        self.assertEquals(
            resolve(url).func.view_class, FactoryCreate.as_view().view_class
        )

    def test_factory_update_page_url(self):
        url = reverse("factory_edit", kwargs={"pk": 2})
        self.assertEquals(
            resolve(url).func.view_class, FactoryEdit.as_view().view_class
        )
