...
from vei_platform.models.legal import (
    LegalEntity,
    StakeHolder,
    LegalEntitySources,
    create_legal_entity,
...
class TestLegalEntity(TestCase):
    ...
    def test_creating_company_with_ceo_and_owner(self):
        ...
        num_new_entities = create_legal_entity(info, source="https://papagal.bg")
        ...
        self.assertEqual(people[0].native_name.upper(), "ИВО ДИМИТРОВ КОЛАРОВ")
        ...
        self.assertEqual(
            person_urls[0].entity.native_name.upper(), "ИВО ДИМИТРОВ КОЛАРОВ"
        )
        ...
        self.assertEqual(company_urls[0].entity.native_name.upper(), "ХОТ - ЛАЙТ")
        ...
        self.assertEqual(holders[0].holder.native_name.upper(), "ИВО ДИМИТРОВ КОЛАРОВ")
        ...
        self.assertEqual(holders[1].holder.native_name.upper(), "ИВО ДИМИТРОВ КОЛАРОВ")
    ...
    def test_creating_company_with_multiple_ceos_and_owners(self):
        ...
        galina = LegalEntity.objects.get(native_name="ГАЛИНА ЛЮБОМИРОВА МИРОЛЕСКОВА")
        rumyana = LegalEntity.objects.get(native_name="РУМЯНА ХРИСТОВА МИРОЛЕСКОВА")
        self.assertEqual(
            StakeHolder.objects.filter(holder=galina)
            .filter(stake_type=StakeHolder.OWNER)[0]
            .percent,
            Decimal(10),
        )

        self.assertEqual(
            StakeHolder.objects.filter(holder=rumyana)
            .filter(stake_type=StakeHolder.OWNER)[0]
            .percent,
            Decimal(20),
        )
    ...
    def test_create_et(self):
        ...
        num_new_entities = create_legal_entity(info, info["source"])

        self.assertEqual(num_new_entities, 2)
        georgi = LegalEntity.objects.get(native_name="ГЕОРГИ ДИМИТРОВ ГЕОРГИЕВ")
        et = LegalEntity.objects.get(native_name="ДИВЕКС-2 - ГЕОРГИ ГЕОРГИЕВ")

        traders = StakeHolder.objects.filter(holder=georgi).filter(
            stake_type=StakeHolder.TRADER
        )

        owners = StakeHolder.objects.filter(holder=georgi).filter(
        ...
        self.assertEqual(owners[0].holder.native_name, "ГЕОРГИ ДИМИТРОВ ГЕОРГИЕВ")
    ...
    def test_create_ead(self):
        ...
        self.assertEqual(num_new_entities, 5)
        self.assertEqual(len(LegalEntity.objects.filter(native_name="ЛЪКИ ЕНЕРГИЯ")), 1)

        holdings = LegalEntity.objects.filter(native_name="ВЕНТЧЪР ЕКУИТИ БЪЛГАРИЯ")
        self.assertEqual(len(holdings), 1)
        self.assertEqual(holdings[0].person, False)

    def test_legal_name_cannonization(self):
        self.assertEqual(
            canonize_name("ВЕНТЧЪР ЕКУИТИ БЪЛГАРИЯ ЕАД"), "ВЕНТЧЪР ЕКУИТИ БЪЛГАРИЯ"
        )
        self.assertEqual(
            canonize_name('"ВЕНТЧЪР ЕКУИТИ БЪЛГАРИЯ" ЕООД'), "ВЕНТЧЪР ЕКУИТИ БЪЛГАРИЯ"
        )
        self.assertEqual(canonize_name('"РИАЛ ТЕХ" ООД'), "РИАЛ ТЕХ")

        self.assertEqual(
            canonize_name('ЕТ "ДИВЕКС - 2 - ГЕОРГИ ГЕОРГИЕВ"'),
            "ДИВЕКС - 2 - ГЕОРГИ ГЕОРГИЕВ",
        )

        self.assertEqual(canonize_name("Миро-М-2008 ЕООД"), "МИРО-М-2008")

        self.assertEqual(
            canonize_name("“ВОДНИ ЕЛЕКТРИЧЕСКИ ЦЕНТРАЛИ – 94” ООД"),
            "ВОДНИ ЕЛЕКТРИЧЕСКИ ЦЕНТРАЛИ – 94",
        )
        self.assertEqual(
            canonize_name('"ГОРУБСО - ЗЛАТОГРАД" АД'), "ГОРУБСО - ЗЛАТОГРАД"
        )

        self.assertEqual(
            canonize_name('"СЕНТРАЛ ХИДРОЕЛЕКТРИК ДЬО БУЛГАРИ"ЕООД'),
            "СЕНТРАЛ ХИДРОЕЛЕКТРИК ДЬО БУЛГАРИ",
        )
        ...
        self.assertEqual(
            canonize_name("ЕВН БЪЛГАРИЯ РЕС ХОЛДИНГ ГМБХ"), "ЕВН БЪЛГАРИЯ РЕС ХОЛДИНГ"
        )

        self.assertEqual(
            canonize_name("„ЕВЕРЕСТ РИНЮЪБЪЛ ЕНЕРДЖИС (БЪЛГАРИЯ)”"),
            "ЕВЕРЕСТ РИНЮЪБЪЛ ЕНЕРДЖИС (БЪЛГАРИЯ)",
        )

        self.assertEqual(
            canonize_name("„АКУО БЪЛГАРИЯ СИ ЕЙЧ БИ“"), "АКУО БЪЛГАРИЯ СИ ЕЙЧ БИ"
        )
        self.assertEqual(canonize_name("„ВЕЦ МАЛУША“"), "ВЕЦ МАЛУША")
        self.assertEqual(canonize_name("„Гардения Холдинг“ ГмбХ"), "ГАРДЕНИЯ ХОЛДИНГ")
        self.assertEqual(
            canonize_name("„БЪЛГАРСКИ ВИК ХОЛДИНГ“"), "БЪЛГАРСКИ ВИК ХОЛДИНГ"
        )
        self.assertEqual(
            canonize_name("„АКУО БЪЛГАРИЯ СИ ЕЙЧ БИ“"), "АКУО БЪЛГАРИЯ СИ ЕЙЧ БИ"
        )
        self.assertEqual(canonize_name('КООПЕРАЦИЯ "ПАРАЛЕЛ 2000"'), "ПАРАЛЕЛ 2000")
        self.assertEqual(
            canonize_name('"УИНДКРАФТ СИМОНСФЕЛД" АГ'), "УИНДКРАФТ СИМОНСФЕЛД"
        )
        self.assertEqual(canonize_name('„ГАЛИТА 67 БГ" EООД'), "ГАЛИТА 67 БГ")
        self.assertEqual(
            canonize_name('„БИЛДИНГ ЕНТЕРПРАЙЗ" EООД'), "БИЛДИНГ ЕНТЕРПРАЙЗ"
        )
        self.assertEqual(canonize_name('ДРУЖЕСТВО ПО ЗЗД "КАРДАМ 2"'), "КАРДАМ 2")

    def test_creating_ex_owners(self):
        info = {
            "href": "/eik/127585860/b9fb",
            "name": "ВТК",
            "legal_form": "ЕООД",
            "latin_name": "VTK",
            "eik": "127585860",
        ...
        ex_owners = StakeHolder.objects.filter(holder=v[0]).filter(
            stake_type=StakeHolder.EX_OWNER
        )
        ...
        self.assertEqual(ex_owners[0].holder.native_name, "ВАЛЕНТИН НЕДЕЛЧЕВ ГРОЗДЕВ")
