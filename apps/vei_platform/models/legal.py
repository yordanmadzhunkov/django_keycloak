...
class LegalEntitySources(TimeStampMixin):
    entity = models.ForeignKey(LegalEntity, on_delete=models.CASCADE, default=1)
...
def create_legal_entity_stakeholders(entity, info, source):
    for p in info["people"]:
        ...
        stakers = (
            StakeHolder.objects.filter(company=entity)
            .filter(stake_type=stake_type)
...
def create_legal_entity(info, source):
    ...
    if len(c) == 0:
        new_company = LegalEntity(
            ...
            latin_name=(
                canonize_name(info["latin_name"]) if "latin_name" in info.keys() else ""
            ),
            legal_form=info["legal_form"] if "legal_form" in info.keys() else None,
        ...
        new_source = LegalEntitySources(
            entity=new_company, url=("%s%s" % (source, info["href"]))
        )
        new_source.save()
    else:
        new_company = c[0]
